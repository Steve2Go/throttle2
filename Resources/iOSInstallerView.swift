...
//                        installationItem(
//                            title: "System Check",
//                            description: "Check system and existing installations",
//                            status: outputContains("System check complete") ? .complete :
//                                    outputContains("Checking system") ? .inProgress : .pending
//                        )
//                        
//                        installationItem(
//                            title: "Dependencies Installation",
//                            description: "Install required packages",
//                            status: outputContains("Successfully installed") ? .complete :
//                                    outputContains("Installing") ? .inProgress : .pending
//                        )
                        ...
                        if !installationOutput.isEmpty {
                            VStack(alignment: .leading) {
                                ...
                                if installationOutput.contains("[Copy Homebrew Install Command]") {
                                    VStack(alignment: .leading, spacing: 8) {
                                        Text(installationOutput.replacingOccurrences(of: "[Copy Homebrew Install Command]", with: ""))
                                            .font(.system(.caption, design: .monospaced))
                                        
                                        Button(action: {
                                            UIPasteboard.general.string = "/bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\" && brew install ffmpeg && brew install transmission-cli"
                                            showToast.toggle()
                                        }) {
                                            HStack {
    ...
    private func performInstallation() async {
        ...
        do {
            ...
            // This is the content of our FFmpeg installer script
            let scriptContent = """
            #!/bin/bash
            ...
            let writeCmd = """
            cat > /tmp/install_ffmpeg.sh << 'ENDOFSCRIPT'
            \(scriptContent)
            ENDOFSCRIPT
            chmod +x /tmp/install_ffmpeg.sh
            """
            ...
            let installCmd = "echo '\(password)' | /tmp/install_ffmpeg.sh"
