   ...
   // let persistenceController = PersistenceController.shared
    ...
    var body: some Scene {
        ...
        Window("Throttle 2", id: "main-window") {
            ContentView(presenting: presenting,manager: manager, filter: filter, store: store,activeSheet: $activeSheet)
                ...
                .onChange(of: store.selection) { oldValue, newValue in
                    ...
                    if let selection = newValue,
                       ...
                       !user.isEmpty {
                        // Create the path for Transmission Server
                        if let password = keychain["password" + selection.name!], !password.isEmpty {
                            store.connectTransmission = (selection.url?.replacingOccurrences(
                                of: "://",
                                with: "://" + user + ":" + password + "@"
                            ) ?? "http://localhost") + (store.selection?.rpc ?? "")
                        } else {
                        ...
                        if selection.pathHttp != "" && selection.pathServer != "" {
                            
                            if let password = keychain["password" + selection.name!], !password.isEmpty {
                                store.connectHttp = (selection.pathHttp?.replacingOccurrences(
                                    of: "://",
                                    with: "://" + user + ":" + password + "@"
                                ))!
                            } else {
                                store.connectHttp = selection.pathHttp ?? ""
                            }
                        }
                        
                        ///Create the filepath
                        if selection.pathFilesystem != "" && selection.pathServer != "" {
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                Button("Servers...") {
                    activeSheet = .servers  // Assuming you have activeSheet enum defined
                }
                .keyboardShortcut("e", modifiers: [.command])
                
                Button("Settings...") {
                    activeSheet = .settings  // Assuming you have activeSheet enum defined
                }
